...
void FAkInitializationStructure::SetPluginDllPath(const FString& PlatformArchitecture)
{
	...
	auto Path = AkUnrealHelper::GetThirdPartyDirectory() / PlatformArchitecture / Configuration / "bin" / "";
	auto Length = Path.Len() + 1;
	PluginDllPath = new AkOSChar[Length];
	AKPLATFORM::SafeStrCpy(PluginDllPath, TCHAR_TO_AK(*Path), Length);
}

void FAkInitializationStructure::SetupLLMAllocFunctions(MemoryAllocFunction alloc, MemoryFreeFunction free)
{
...
namespace FAkSoundEngineInitialization
{
	bool Initialize(CAkUnrealIOHookDeferred* IOHookDeferred)
	{
		...
		{
			UE_LOG(LogAkAudio, Error, TEXT("IOHookDeferred is null."));
			return false;
		}
